    ...
    def forward(self, A, X, target_x, target):
        ...
        for i in range(self.num_channels):
            if i == 0:
                ...
                ### --- Hint ---
                ### X(node features) and H[i](i-th weighted meta-paths) are passed to gcn_conv -> ReLU to calculate X_.
            else:
                # TODO 1 : Fill in models.py
                ### --- Hint ---
                ### if i != 0, channelwise concatenation is applied with same procedure in i == 0.
        X_ = self.linear1(X_)
        X_ = F.relu(X_)
...
class GTLayer(nn.Module):
    ...
    def forward(self, A, H_=None):
        if self.first == True:
            ...
            # TODO 1 : Fill in models.py
            ### --- Hint ---
            ### H is newly created meta-paths using a, b, W is softmax-applied convolutional filters.
        else:
            a = self.conv1(A)

            # TODO 1 : Fill in models.py
            ### --- Hint ---
            ### H is newly created meta-paths using a, and previously calculated H_, W is a softmax-applied convolutional filter.
        return H,W
